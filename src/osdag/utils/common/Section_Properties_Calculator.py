...
def get_section_properties(section_type, section_size):
    """
    Returns a dictionary of section properties based on section type and size.
    
    Args:
        section_type (str): Type of section (e.g., "ISMB", "ISMC", "ISA")
        section_size (str): Size designation (e.g., "ISMB 100", "ISMC 75")
        
    Returns:
        dict: Dictionary containing section properties
    """
    # Create appropriate section object based on type
    if section_type == "ISMB":
        section = I_sectional_Properties()
        # Extract dimensions from size string (e.g., "ISMB 100" -> D=100)
        D = float(section_size.split()[1])
        B = D * 0.5  # Approximate flange width
        t_w = D * 0.05  # Approximate web thickness
        t_f = D * 0.08  # Approximate flange thickness
        
        return {
            'A': section.calc_Area(D, B, t_w, t_f),
            'Iyy': section.calc_MomentOfAreaY(D, B, t_w, t_f),
            'Izz': section.calc_MomentOfAreaZ(D, B, t_w, t_f),
            'Cyy': B/2,  # Distance from centroid to flange edge
            'bf': B  # Flange width
        }
        
    elif section_type == "ISMC":
        section = Single_Channel_Properties()
        # Extract dimensions from size string (e.g., "ISMC 75" -> D=75)
        D = float(section_size.split()[1])
        B = D * 0.4  # Approximate flange width
        t_w = D * 0.05  # Approximate web thickness
        t_f = D * 0.08  # Approximate flange thickness
        ...
        return {
            ...
            'Iyy': section.calc_MomentOfAreaY(D, B, t_w, t_f),
            'Izz': section.calc_MomentOfAreaZ(D, B, t_w, t_f),
            'Cyy': section.calc_C_y(D, B, t_w, t_f),
            'bf': B  # Flange width
        }
        
    elif section_type == "ISA":
        section = Single_Angle_Properties()
        # Extract dimensions from size string (e.g., "ISA 40x40x5" -> a=40, b=40, t=5)
        dims = section_size.split()[1].split('x')
        a = float(dims[0])
        b = float(dims[1])
        t = float(dims[2])
        
        return {
            'A': section.calc_Area(a, b, t, 'Long Leg'),
            'Iyy': section.calc_MomentOfAreaY(a, b, t, 'Long Leg'),
            'Izz': section.calc_MomentOfAreaZ(a, b, t, 'Long Leg'),
            'Cyy': section.calc_Cy(a, b, t, 'Long Leg'),
            'bf': max(a, b)  # Maximum leg length
        }
